...
void *pth_srt(void *arg) {
    struct pth_arg *args = (struct pth_arg *)arg;    
    ...
    /* for every pixel */
    for( int px=args->px_start; px<args->px_end; ++px )
    {
...
main( int argc, char **argv )
{
    ...
    for (int i = 0; i < nthreads; i++) {
      int remainder = (i + 1 == nthreads) ? width % nthreads : 0;
      struct pth_arg *arg;
      arg = malloc(sizeof(struct pth_arg));
      if (arg == NULL) {
        fprintf(stderr, "Error when allocating space for thread arguments.");       
      }
      arg->scene = &scene;
      arg->px_start = (width / nthreads) * i;
      arg->px_end = (width / nthreads) * (i + 1) + remainder;
      int ret = pthread_create(&threads[i], NULL, pth_srt, (void *)arg);       
      if (ret) {
        fprintf(stderr, "Error when creating a new thread");
        exit(1);
      } 
    }
    ...
    for (int px = 0; px < width; ++px) {
      for (int py = 0; py < height; ++py) {
        printf("%.0f %.0f %.0f\n", image[px][py][0], image[px][py][1], image[px][py][2]);
      }
      printf("\n");
    }
